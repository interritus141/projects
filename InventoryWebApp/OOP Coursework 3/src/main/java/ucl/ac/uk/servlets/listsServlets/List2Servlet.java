...
@WebServlet("/list2servlet.html")
public class List2Servlet extends HttpServlet {
    private final ItemList list2Items;
    private String name;

    public List2Servlet() {
        this.list2Items = new ItemList();
        loadFile();
    }
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        RequestDispatcher dispatch = context.getRequestDispatcher("/ItemLists/itemlist2.jsp");
        dispatch.forward(request, response);
    }


    private void createNewFile(HttpServletRequest request) {
        name = (String) request.getSession().getAttribute("List2");
        try {
            list2Items.saveToFile(name);
        } catch (IOException e) {
    ...
    private void setAttributes(HttpServletRequest request) {
        request.setAttribute("itemName", getListOfItemAttributes(list2Items, "Name"));
        request.setAttribute("itemIndex", getListOfItemAttributes(list2Items, "Index"));
        request.setAttribute("itemDescription", getListOfItemAttributes(list2Items, "Description"));
        request.setAttribute("itemURL", getListOfItemAttributes(list2Items, "URL"));
        request.setAttribute("itemImage", getListOfItemAttributes(list2Items, "Image"));
        request.setAttribute("itemImageURL", getListOfItemAttributes(list2Items, "ImageURL"));
    }
