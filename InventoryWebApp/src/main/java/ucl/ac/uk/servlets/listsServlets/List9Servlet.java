...
@WebServlet("/list9servlet.html")
public class List9Servlet extends HttpServlet {
    private final ItemList list9Items;
    private String name;

    public List9Servlet() {
        this.list9Items = new ItemList();
        loadFile();
    }
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        RequestDispatcher dispatch = context.getRequestDispatcher("/ItemLists/itemlist9.jsp");
        dispatch.forward(request, response);
    }


    private void createNewFile(HttpServletRequest request) {
        name = (String) request.getSession().getAttribute("List9");
        try {
            list9Items.saveToFile(name);
        } catch (IOException e) {
    ...
    private void setAttributes(HttpServletRequest request) {
        request.setAttribute("itemName", getListOfItemAttributes(list9Items, "Name"));
        request.setAttribute("itemIndex", getListOfItemAttributes(list9Items, "Index"));
        request.setAttribute("itemDescription", getListOfItemAttributes(list9Items, "Description"));
        request.setAttribute("itemURL", getListOfItemAttributes(list9Items, "URL"));
        request.setAttribute("itemImage", getListOfItemAttributes(list9Items, "Image"));
        request.setAttribute("itemImageURL", getListOfItemAttributes(list9Items, "ImageURL"));
    }
